...
- **A component "exists"** — is discoverable by future tooling (the
  registry, the generator's own validation) — **if and only if its
  directory exports a default component from `index.ts`.** This is the
  same discovery convention the documentation-system design already
  committed to; this ADR just confirms it holds at the folder-structure
  level too.

## Consequences

`internal/` is a real API boundary, not just a naming convention — code
there is not meant to be imported by consumers, even though nothing
currently enforces that beyond it not being re-exported from the barrel.
